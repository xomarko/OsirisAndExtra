...
[[nodiscard]] json toJson(const inventory_changer::inventory::Skin& skin)
{
    ...
    if (skin.tournamentID != Tournament{})
...
[[nodiscard]] std::optional<inventory_changer::backend::PickEm::PickPosition> pickPositionFromJson(const json& j)
{
    ...
    return inventory_changer::backend::PickEm::PickPosition{ static_cast<Tournament>(tournament->get<std::uint8_t>()), group->get<std::uint16_t>(), indexInGroup->get<std::uint8_t>() };
}
