    ...
    void Items::getStickers(game_items::Storage& storage)
    {
        ...
        for (const auto& node : stickerMap) {
            ...
            if (isSticker) {
                ...
                storage.addSticker(stickerKit->id, game_items::ItemName{ toUtf8.convertUnicodeToAnsi(stickerName), toUpperConverter.toUpper(stickerName) }, static_cast<EconRarity>(stickerKit->rarity), stickerKit->inventoryImage.data(), static_cast<Tournament>(stickerKit->tournamentID), static_cast<TournamentTeam>(stickerKit->tournamentTeamID), stickerKit->tournamentPlayerID, isGolden);
            }
            else if (isPatch) {
        ...
        for (const auto& node : itemSchema.itemsSorted) {
            ...
            if (const auto weaponID = item->getWeaponId(); itemTypeName == "#CSGO_Type_Knife" && rarity == EconRarity::Red) {
                ...
                if (item->isServiceMedal()) {
                    ...
                    storage.addTournamentCoin(rarity, weaponID, static_cast<Tournament>(item->getTournamentEventID()), static_cast<std::uint16_t>(item->getStickerID()), inventoryImage);
                }
                else {
                ...
                storage.addCrate(rarity, weaponID, static_cast<std::uint16_t>(item->getCrateSeriesNumber()), static_cast<Tournament>(item->getTournamentEventID()), getTournamentMapOfSouvenirPackage(baseName), baseName.find("promo") != std::string_view::npos, inventoryImage);
            }
                ...
                if (std::strcmp(tool->typeName, "season_pass") == 0)
                    ...
                    if (Helpers::isSouvenirToken(weaponID))
                        storage.addSouvenirToken(rarity, weaponID, static_cast<Tournament>(item->getTournamentEventID()), inventoryImage);
                    else
                        storage.addViewerPass(rarity, weaponID, static_cast<Tournament>(item->getTournamentEventID()), inventoryImage);
                }
