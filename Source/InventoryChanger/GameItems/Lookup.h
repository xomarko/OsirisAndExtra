...
namespace inventory_changer::game_items
{
    ...
    private:
        auto findTournamentStickers(Tournament tournament) const noexcept
        {
    ...
    public:
        int findTournamentEventStickerID(Tournament tournament) const noexcept
        {
            if (tournament == Tournament::DreamHack2013)
                return Helpers::random(1, 12);
            else if (tournament == Tournament::EmsOneKatowice2014)
                return Helpers::random(99, 100); // EMS Wolf / Skull
            else if (tournament == Tournament::EslOneCologne2014)
                return 172;
        ...
        int findTournamentTeamGoldStickerID(Tournament tournament, TournamentTeam team) const noexcept
        {
            if (tournament == Tournament{} || team == TournamentTeam::None)
                return 0;
        ...
        int findTournamentPlayerGoldStickerID(Tournament tournament, ProPlayer player) const noexcept
        {
            const auto range = findTournamentStickers(tournament);
