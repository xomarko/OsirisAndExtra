...
namespace inventory_changer::game_items
{
    ...
    public:
        ...
        void addSticker(int id, ItemName name, EconRarity rarity, std::string_view iconPath, Tournament tournament, TournamentTeam tournamentTeam, int tournamentPlayerID, bool isGoldenSticker);
        ...
        void addTournamentCoin(EconRarity rarity, WeaponId weaponID, Tournament tournament, std::uint16_t defaultGraffitiID, std::string_view iconPath);
        ...
        void addCrate(EconRarity rarity, WeaponId weaponID, std::uint16_t crateSeries, Tournament tournament, TournamentMap map, bool isSouvenirPackage, std::string_view iconPath);
        ...
        void addSouvenirToken(EconRarity rarity, WeaponId weaponID, Tournament tournament, std::string_view iconPath);
        void addViewerPass(EconRarity rarity, WeaponId weaponID, Tournament tournament, std::string_view iconPath);
        ...
        [[nodiscard]] Tournament getTournamentEventID(const Item& item) const noexcept
        {
            assert(item.isSouvenirToken() || item.isViewerPass() || item.isTournamentCoin() || item.isCrate());
            return static_cast<Tournament>(item.getDataIndex() & 0xFF);
        }
